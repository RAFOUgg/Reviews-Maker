## Trame temporelle de la pipeline

- Heures  
- Jours  
- Dates calendaires  
- Semaines  
- Mois  
- Phases physiologiques  

### Phases physiologiques (exhaustif, modulaire)

- 0 day / Graine (seed)  
- Germination  
- Plantule / Seedling  
- Croissance végétative – Début  
- Croissance végétative – Milieu  
- Croissance végétative – Fin  
- Stretch – Début  
- Stretch – Milieu  
- Stretch – Fin  
- Floraison – Début  
- Floraison – Milieu  
- Floraison – Fin  
- Maturation / Fin de floraison  
- Rinçage / Flush  
- Séchage (post‑récolte)  
- Affinage / Curing  

***

## Section [GENERAL]

### Définition des phases

- Pipeline par temps (heures/jours/semaines/mois)  
- Pipeline par phases physiologiques  
- Pipeline hybride (temps + phases)  

### Début / fin de culture

- Date (sélection calendrier, champ libre)  

### Mode de culture

- Indoor (intérieur)  
- Outdoor (extérieur plein champ)  
- Greenhouse / Serre chauffée  
- Greenhouse froide / non chauffée  
- Greenhouse avec assistance lumineuse  
- No‑till indoor  
- No‑till outdoor  
- Culture en container / bac hors‑sol  
- Culture verticale (multi‑niveaux)  
- Culture mixte (indiquer combinaison)  

### Espace de culture – Type

- Armoire de culture  
- Tente de culture  
- Chambre dédiée  
- Pièce industrielle / salle blanche  
- Serre verre  
- Serre polycarbonate  
- Tunnel plastique  
- Plein champ extérieur  
- Balcon / terrasse  
- Box DIY / caisson technique  
- Autre (texte libre)  

### Dimensions / Surface / Volume

- Longueur (cm ou m) – nombre libre  
- Largeur (cm ou m) – nombre libre  
- Hauteur (cm ou m) – nombre libre  
- Surface au sol (m²) – calculée ou saisie  
- Volume total (m³) – calculé ou saisi  

***

## Section [ENVIRONNEMENT]

### Technique de propagation

- Graine directement en substrat  
- Graine en pastille Jiffy / tourbe  
- Graine en laine de roche  
- Graine en cube Root Riot / similaire  
- Germination sur sopalin  
- Germination sur coton  
- Germination dans serviette humide  
- Germination en eau (verre d’eau)  
- Germination en propagateur chauffant  
- Clone / bouture prélevée en interne  
- Clone acheté / externe  
- Bouture en eau claire  
- Bouture en eau + hormone d’enracinement  
- Bouture en substrat (tourbe/terre/coco)  
- Bouture en laine de roche  
- Bouture en cube de propagation (Root Riot, etc.)  
- Micropropagation / in vitro (rare mais possible pro)  
- Autre (texte libre)  

***

## Substrat

### Type de substrat

- Hydroponique recirculé  
- Hydroponique drain‑to‑waste  
- DWC (deep water culture)  
- RDWC (recirculating DWC)  
- NFT (nutrient film technique)  
- Aéroponie haute pression  
- Aéroponie basse pression  
- Substrat inerte (coco, laine de roche, billes d’argile, perlite, vermiculite)  
- Terreau « Bio »  
- Terreau organique vivant (living soil)  
- Super‑soil / no‑till  
- Mélange terre / coco  
- Mélange terre / perlite  
- Mélange coco / perlite  
- Mélange coco / billes d’argile  
- Mélange personnalisé (composition précisée)  

### Volume de substrat

- Volume par contenant (L) – champ numérique  
- Volume total de substrat (L) – optionnel  

### Composition (en %)

Prévoir des listes de composants sélectionnables, avec % en saisie numérique :

- Terre végétale  
- Terreau horticole  
- Terreau spécial cannabis  
- Coco pur  
- Coco tamponnée  
- Laine de roche  
- Perlite  
- Vermiculite  
- Billes d’argile  
- Pouzzolane  
- Sable  
- Biochar  
- Compost maison  
- Compost lombricompost  
- Humus / moulm  
- Tourbe blonde  
- Tourbe brune  
- Amendements minéraux (dolomie, basalte, etc.)  
- Amendements organiques (guano, farine d’os, etc.)  

### Marques (sélection + texte libre)

- Canna  
- Plagron  
- Biobizz  
- Atami  
- Advanced Nutrients  
- House & Garden  
- General Hydroponics / Terra Aquatica  
- FoxFarm  
- Mills  
- Green House Feeding  
- BAC  
- Aptus  
- Remo  
- Autre (texte libre pour tout ingrédient)  

[1][2][3]

***

## Système d’irrigation

### Type de système

- Arrosage manuel (arrosoir)  
- Arrosage manuel + pompe électrique  
- Goutte à goutte simple  
- Goutte à goutte avec piquets  
- Goutte à goutte réglable  
- Goutte à goutte multi‑sorties  
- Ligne de goutte à goutte (drip line)  
- Ebb & Flow / Flood & Drain (table inondation)[4][5]
- Sub‑irrigation / bottom feeding (plateaux, SIP)[6]
- Systèmes top‑feed automatisés  
- Systèmes à mèche / wicking[6]
- DWC avec bullage  
- RDWC  
- NFT  
- Aéroponie basse pression  
- Aéroponie haute pression  
- Système hybride (préciser)  
- Brumisation / fogponic  

### Fréquence d’arrosage (sélection + champ)

- Par jour : 1 / 2 / 3 / 4 / 6 / 8 / 12 fois  
- Par semaine : 1 / 2 / 3 / 4 / 5 / 6 / 7 fois  
- Irrigation en continu  
- Irrigation à la demande (capteurs, tensiomètre)  

### Volume d’eau par arrosage

- Champ numérique (L ou mL)  
- Mode :  
  - Volume fixe par pot  
  - Volume fixe par m²  
  - Volume variable (commentaire)  

[7][8][4][6]

***

## Engrais utilisés

### Type d’engrais

- Minéral / chimique  
- Organique  
- Organique‑minéral / mixte  
- Biologique certifié  
- Amendement solide à libération lente  
- Solution nutritive hydroponique  
- Booster / stimulateur racinaire  
- Booster floraison  
- Additifs (enzymes, sucres, PK, etc.)  

### Marque / gamme (sélection + libre)

- Canna (Terra, Aqua, Coco, BioCanna, etc.)  
- Plagron (Terra, Alga, Coco, etc.)  
- Biobizz (Bio‑Grow, Bio‑Bloom, etc.)  
- Advanced Nutrients (pH Perfect, etc.)  
- General Hydroponics / Terra Aquatica  
- House & Garden  
- Green House Feeding  
- FoxFarm  
- Mills  
- Aptus  
- Remo Nutrients  
- Autre (texte libre)  

### Dosage

- Unité : g/L, ml/L, ml/10 L, EC cible, ppm cible  
- Champ numérique  

### Fréquence d’application (sélection)

- À chaque arrosage  
- 1 arrosage sur 2  
- 1 fois par jour  
- 1 fois tous les 2 jours  
- 1 fois par semaine  
- 1 fois toutes les 2 semaines  
- Continu (fertigation permanente)  
- Ponctuel (début stretch, etc.)  

Échelle de temps (pour paramétrage) :

- Seconde  
- Minute  
- Heure  
- Jour  
- Semaine  

***

## Lumière

### Type de lampe

- LED panneau  
- LED barre linéaire  
- LED quantum board  
- LED COB  
- HPS (sodium haute pression)  
- MH (métal halide)  
- CMH / LEC  
- CFL / néon / T5  
- Plasma  
- Halogénure céramique  
- Multibar industrial LED  
- Lumière naturelle uniquement  
- Lumière naturelle + complément LED  
- Lumière naturelle + HPS  
- Autre (texte libre)  

### Spectre

- Spectre complet  
- Dominante bleue  
- Dominante rouge  
- Croissance (blue heavy)  
- Floraison (red heavy)  
- UV‑A inclus  
- UV‑B inclus  
- IR / Far‑red inclus  
- Spectre ajustable / multi‑canaux  
- Non spécifié  

### Distance lampe / plante

- Unité : cm / m / pouces  
- Mode :  
  - Fixe  
  - Variable (suivi dans pipeline)  

### Puissance

- Puissance totale (W) – champ numérique  
- Puissance par m² (W/m²) – calculable  
- Dimmable : oui / non  

### Photopériode / durée d’éclairage

- 24/0  
- 20/4  
- 18/6  
- 16/8  
- 12/12  
- 11/13  
- 10/14  
- Personnalisée (heures ON/OFF libres)  

### Paramètres lumineux avancés

- DLI disponible : oui / non ; valeur (mol/m²/jour) – numérique  
- PPFD moyen (µmol/m²/s) – numérique  
- Kelvin (température de couleur) :  
  - 2700 K  
  - 3000 K  
  - 3500 K  
  - 4000 K  
  - 5000 K  
  - 6500 K  
  - Spectre mixte / non applicable  

[2][3][9]

***

## Environnement climatique

### Température

- Température moyenne (°C) – champ numérique  
- Plage cible :  
  - Végétatif jour : 22–30 °C (sélections prédéfinies)  
  - Végétatif nuit : 18–24 °C  
  - Floraison jour : 20–28 °C  
  - Floraison nuit : 16–22 °C[3][1][2]
- Mode : contrôlée / non contrôlée  

### Humidité relative

- HR moyenne (%) – champ numérique  
- Plages typiques :  
  - Germination : 70–90%  
  - Plantule : 65–80%  
  - Croissance : 55–80%  
  - Floraison : 40–60%  
  - Fin de floraison : 35–50%[10][1]

### CO₂

- Non enrichi (~400–500 ppm)  
- CO₂ enrichi :  
  - 600–800 ppm  
  - 800–1000 ppm  
  - 1000–1200 ppm  
  - >1200 ppm (avancé)[1][3]
- Mode : injection bouteille, générateur, HVACD, pas de contrôle  

### Ventilation

- Type de ventilation :  
  - Extracteur d’air  
  - Intracteur d’air  
  - Ventilateur oscillant  
  - Ventilation au plafond  
  - Ventilation par gaines (HVACD)  
  - Déshumidificateur  
  - Humidificateur  
  - Filtre à charbon  
- Mode :  
  - Continu  
  - Cyclé (minuterie)  
  - Piloté par hygromètre/thermostat  
  - Piloté par pression différentielle  

[9][11][12][3][10]

***

## Palissage / Training (LST/HST)

### Méthodologies (liste exhaustive de sélection multiple)

- LST (Low Stress Training)  
- HST (High Stress Training)  
- Topping (étêtage)  
- Fimming  
- Main‑Lining / Manifolding[13][14][15]
- SCROG (Screen of Green)[15][16][13]
- SOG (Sea of Green)[16][17][13]
- Lollipopping  
- Super‑cropping  
- Defoliation ciblée  
- Super‑cropping + support tuteur / filet  
- Splitting / fente de tige (avancé)  
- Tuteurs individuels  
- Filets multi‑niveaux  
- Palissage horizontal  
- Palissage vertical  
- Ligaturage / tie‑down simple  
- Ligaturage en étoile  
- Taille apicale répétée  
- Taille latérale  
- Taille de racines (rares, hydro)  
- Pas de palissage  
- Autre technique (texte libre)  

### Commentaire de palissage

- Champ texte libre (description de la manipulation)  

[17][18][13][15][16]

***

## Morphologie de la plante

### Taille

- Unité : cm / m  
- Catégories possibles pour sélection rapide :  
  - <30 cm  
  - 30–60 cm  
  - 60–90 cm  
  - 90–120 cm  
  - 120–150 cm  
  - 150–200 cm  
  - >200 cm  

### Volume

- Volume de canopée (approx.)  
  - Petit  
  - Moyen  
  - Grand  
  - Très volumineux  
- Volume chiffré (m³) – champ numérique libre  

### Poids de la plante fraîche (hors racines)

- Champ numérique (g/kg)  

### Nombre de branches principales

- Champ numérique  
- Catégories rapides : 1–4 / 5–8 / 9–12 / >12  

### Nombre de feuilles (optionnel, estimé)

- Champ numérique ou classes :  
  - <50  
  - 50–100  
  - 100–200  
  - >200  

### Nombre de buds / sites floraux

- Champ numérique  
- Catégories :  
  - <20  
  - 20–50  
  - 50–100  
  - >100  

***

## Récolte

### Couleur des trichomes

Prévoir sélection multiple + % :

- Transparent / translucide  
- Laiteux / opaque  
- Ambré  
- Mélange transparent–laiteux  
- Mélange laiteux–ambré  
- Majorité laiteux  
- Majorité ambré  

### Date de récolte

- Date calendrier  

### Poids

- Poids brut (plante entière fraîche) – g  
- Poids net après première défoliation – g  
- Poids sec final (optionnel) – g  
- Taux de perte (%) – calculable  

### Rendement

- g/m² – champ numérique  
- g/plante – champ numérique  
- g/W – optionnel  
- Catégories qualitatives :  
  - Faible  
  - Moyen  
  - Bon  
  - Très élevé  
