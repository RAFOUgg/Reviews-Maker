...
export default function CreateReviewPage() {
    ...
    const renderField = (field) => {
        ...
        switch (field.type) {
            ...
            case 'slider': return <div><input type="range" min="0" max={field.max || 10} step="0.5" value={value} onChange={(e) => handleInputChange(field.key, parseFloat(e.target.value))} className="w-full h-3 rounded-lg appearance-none cursor-pointer bg-gradient-to-r from-purple-500 to-purple-300 dark:from-purple-600 dark:to-purple-400 shadow-lg border-2 border-purple-400 dark:border-purple-500" /><div className="flex justify-between items-center mt-2"><span className="text-xs text-white/70 font-bold">0</span><span className="text-2xl font-bold text-white glow-text-subtle">{value}/{field.max || 10}</span><span className="text-xs text-white/70 font-bold">{field.max || 10}</span></div></div>;
            case 'select': return <select value={value} onChange={(e) => handleInputChange(field.key, e.target.value)} className="w-full px-4 py-3 bg-gray-900/95 dark:bg-gray-800/95 border-2 border-purple-400/50 dark:border-purple-500/50 rounded-xl text-white font-medium focus:outline-none focus:border-purple-400 dark:focus:border-purple-500 shadow-lg glow-container-subtle" style={{ backgroundImage: 'none' }}><option value="" className="bg-gray-900 text-white">-- Sélectionner --</option>{field.choices?.map((choice, i) => <option key={i} value={choice} className="bg-gray-900 text-white py-2">{choice}</option>)}</select>;
