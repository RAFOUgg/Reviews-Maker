...
const PipelineCell = ({
    ...
    return (
        <motion.div
            ...
            className={`relative group cursor-pointer ${isPhaseMode ? 'w-8 h-8 md:w-10 md:h-10' : 'w-3 h-3 md:w-3.5 md:h-3.5'} rounded-sm ${getIntensityColor(intensity)} border transition-all duration-200 hover:border-white/10 hover:shadow-lg hover:shadow-black/40 hover:ring-2 hover:ring-white/6`}
        ...
        >
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
                <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 shadow-xl whitespace-nowrap border border-gray-700">
                    <div className="font-bold mb-1">{getLabel()}</div>
                    {data ? (
                        <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {CULTURE_PHASES.map((phase, idx) => (
                            ...
                            >
                                ...
                                {/* Badge intensité */}
                                <div className={`absolute top-2 right-2 w-3 h-3 rounded-full ${getCellIntensity(idx) > 0 ? 'bg-green-400' : 'bg-gray-600'}`} />
                            </motion.div>
                        ))}
                    </div>
                ) : (
