...
export default function ToastContainer() {
    ...
    const getColors = (type) => {
        switch (type) {
            case 'success':
                return 'bg-green-500/20 border-green-500/50 text-green-50'
            case 'error':
                return 'bg-red-500/20 border-red-500/50 text-red-50'
            case 'warning':
    ...
    return (
        <div className="fixed top-4 right-4 z-[9999] space-y-2 max-w-sm">
            {toasts.map((toast) => (
                <div
                    key={toast.id}
                    className={`flex items-start gap-3 p-4 rounded-xl border backdrop-blur-xl shadow-2xl animate-slide-in-right ${getColors(toast.type)}`}
