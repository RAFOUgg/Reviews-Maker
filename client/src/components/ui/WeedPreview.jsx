...
const WeedPreview = ({
    ...
    const bracts = useMemo(() => {
        ...
        for (let layer = 0; layer < layers; layer++) {
            ...
            for (let i = 0; i < bractCount; i++) {
                ...
                // 0 = très écarté, 10 = ultra compact
                const radiusVariation = 1 + (1 - compactness) * 0.8;
                ...
                const size = baseSize * (1.4 - compactness * 0.6);
    ...
    const pistilStrands = useMemo(() => {
        const items = [];
        // 0 = 1 pistil par bractée max, 10 = 4+ pistils par bractée
        const pistilsPerBract = pistils === 0 ? 0.2 : Math.max(1, Math.ceil((pistils / 10) * 4.5));

        bracts.forEach((bract, bractIdx) => {
            const pistilCount = pistils === 0
                ? (bractIdx % 5 === 0 ? 1 : 0)
                : Math.floor(pistilsPerBract);

            for (let p = 0; p < pistilCount; p++) {
                const angle = Math.atan2(bract.y - 150, bract.x - 120) + (p * 0.4 - 0.4);
                const length = 18 + pistils * 2.5;
                const curl = Math.sin(bractIdx * 0.8 + p) * 10;
    ...
    const trichomeGlands = useMemo(() => {
        const items = [];
        // 0 = 1 trichome par bractée max, 10 = 6+ trichomes par bractée
        const trichomesPerBract = trichomes === 0 ? 0.3 : Math.max(1, Math.ceil((trichomes / 10) * 7));

        bracts.forEach((bract, bractIdx) => {
            const trichomeCount = trichomes === 0
                ? (bractIdx % 3 === 0 ? 1 : 0)
                : Math.floor(trichomesPerBract);
