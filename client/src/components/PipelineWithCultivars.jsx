    ...
    return (
        <div className="space-y-4">
            {pipeline.length > 0 && (
                <ol className="space-y-3">
                    {pipeline.map((step, idx) => (
                        <li key={step.id} className="bg-gray-800/80 border-2 border-purple-500/30 rounded-xl p-3 shadow-lg">
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {cultivarsList.map((cultivar, i) => {
                                            ...
                                            return <label key={i} className={`px-3 py-1.5 rounded-lg text-sm cursor-pointer transition-all border-2 ${isChecked ? 'bg-purple-600/30 border-purple-400 text-white font-bold shadow-lg glow-text-subtle' : 'bg-gray-700/50 border-gray-600 text-gray-300 hover:border-purple-500/50 hover:bg-gray-700/80'}`}><input type="checkbox" className="hidden" checked={isChecked} onChange={() => toggleCultivar(step.id, cultivar.name)} />{cultivar.name}</label>;
                                        })}
                                    </div>
                                </div>
                            )}
                            <div className="space-y-2">
                                {(isSieveStep(step.name) || isRosinStep(step.name)) && (<div className="flex gap-2"><input type="text" placeholder="min (µm)" value={step.meshMin || ''} onChange={(e) => updateStep(step.id, 'meshMin', e.target.value)} className="flex-1 px-3 py-1.5 bg-gray-700/80 border-2 border-purple-500/30 rounded-lg text-white text-sm focus:outline-none focus:border-purple-400 shadow-inner" /><input type="text" placeholder="max (µm)" value={step.meshMax || ''} onChange={(e) => updateStep(step.id, 'meshMax', e.target.value)} className="flex-1 px-3 py-1.5 bg-gray-700/80 border-2 border-purple-500/30 rounded-lg text-white text-sm focus:outline-none focus:border-purple-400 shadow-inner" /></div>)}
                                {isRosinStep(step.name) && (<input type="text" placeholder="Température (°C)" value={step.tempC || ''} onChange={(e) => updateStep(step.id, 'tempC', e.target.value)} className="w-full px-3 py-1.5 bg-gray-700/80 border-2 border-purple-500/30 rounded-lg text-white text-sm focus:outline-none focus:border-purple-400 shadow-inner" />)}
                                {isCO2Step(step.name) && (<div className="flex gap-2"><input type="text" placeholder="Pression (bar)" value={step.pressureBar || ''} onChange={(e) => updateStep(step.id, 'pressureBar', e.target.value)} className="flex-1 px-3 py-1.5 bg-gray-700/80 border-2 border-purple-500/30 rounded-lg text-white text-sm focus:outline-none focus:border-purple-400 shadow-inner" /><input type="text" placeholder="Température (°C)" value={step.tempC || ''} onChange={(e) => updateStep(step.id, 'tempC', e.target.value)} className="flex-1 px-3 py-1.5 bg-gray-700/80 border-2 border-purple-500/30 rounded-lg text-white text-sm focus:outline-none focus:border-purple-400 shadow-inner" /></div>)}
                            </div>
                        </li>
                    ))}
                </ol>
            ...
            <div className="relative">
                ...
                {showMenu && hasValidCultivars && (<><div className="fixed inset-0 z-10" onClick={() => setShowMenu(false)} /><div className="absolute top-full left-0 right-0 mt-2 bg-gray-900/98 backdrop-blur-xl border-2 border-purple-500/50 rounded-xl shadow-2xl max-h-64 overflow-y-auto z-20">{choices.map((choice, i) => (<button key={i} type="button" onClick={() => addStep(choice)} className="w-full px-4 py-2.5 text-left text-sm font-medium text-white hover:bg-purple-600/30 hover:text-purple-200 transition-colors border-b border-purple-500/20 last:border-b-0">{choice}</button>))}</div></>)}
            </div>
