# Système de Permissions - Matrice Complète

## 📋 Types de Comptes

### 1. **Amateur** (Gratuit)
- Accès basique aux fonctionnalités
- Templates prédéfinis limités
- Export qualité standard

### 2. **Producteur** (Payant 29.99€/mois)
- Accès complet à toutes les fonctionnalités
- Templates personnalisés
- Export haute qualité
- Pipelines complètes
- Arbre généalogique (Fleurs)
- Statistiques avancées

### 3. **Influenceur** (Payant 15.99€/mois)
- Accès aux aperçus détaillés
- Export haute qualité
- Focus sur rendu visuel
- Drag & drop configuré

---

## 🔐 Matrice des Permissions Détaillée

### CREATE_REVIEWS - Création de Reviews

#### Fleurs
| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Section Infos Générales | ✓ | ✓ | ✓ |
| Section Génétiques | ✓ | ✓ | ✓ |
| Pipeline Culture | ✗ | ✓ | ✓ (lecture) |
| Visuel & Technique | ✓ | ✓ | ✓ |
| Odeurs | ✓ | ✓ | ✓ |
| Texture | ✓ | ✓ | ✓ |
| Goûts | ✓ | ✓ | ✓ |
| Effets Ressentis | ✓ | ✓ | ✓ |
| Pipeline Curing | ✗ | ✓ | ✓ (lecture) |
| Arbre Généalogique | ✗ | ✓ | ✗ |

#### Hash
| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Section Infos Générales | ✓ | ✓ | ✓ |
| Pipeline Séparation | ✗ | ✓ | ✓ (lecture) |
| Visuel & Technique | ✓ | ✓ | ✓ |
| Odeurs | ✓ | ✓ | ✓ |
| Texture | ✓ | ✓ | ✓ |
| Goûts | ✓ | ✓ | ✓ |
| Effets Ressentis | ✓ | ✓ | ✓ |
| Pipeline Curing | ✗ | ✓ | ✓ (lecture) |

#### Concentrés
| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Section Infos Générales | ✓ | ✓ | ✓ |
| Pipeline Extraction | ✗ | ✓ | ✓ (lecture) |
| Pipeline Purification | ✗ | ✓ | ✗ |
| Visuel & Technique | ✓ | ✓ | ✓ |
| Odeurs | ✓ | ✓ | ✓ |
| Texture | ✓ | ✓ | ✓ |
| Goûts | ✓ | ✓ | ✓ |
| Effets Ressentis | ✓ | ✓ | ✓ |
| Pipeline Curing | ✗ | ✓ | ✓ (lecture) |

#### Comestibles
| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Section Infos Générales | ✓ | ✓ | ✓ |
| Pipeline Recette | ✓ | ✓ | ✓ |
| Goûts | ✓ | ✓ | ✓ |
| Effets Ressentis | ✓ | ✓ | ✓ |

---

### BIBLIOTHEQUE - Gestion Ressources

| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Voir reviews sauvegardées | ✓ | ✓ | ✓ |
| Éditer reviews | ✓ | ✓ | ✓ |
| Supprimer reviews | ✓ | ✓ | ✓ |
| Cultiv. & Génétiques | ✗ | ✓ | ✗ |
| Templates prédéfinis | ✓ | ✓ | ✓ |
| Template personnalisé | ✗ | ✓ | ✓ |
| Filigranes | ✓ (limité) | ✓ | ✓ (limité) |
| Données récurrentes | ✗ | ✓ | ✗ |

---

### EXPORT MAKER - Export & Personnalisation

#### Formats
| Format | Amateur | Producteur | Influenceur |
|--------|---------|-----------|------------|
| PNG | Qualité std | 300dpi | 300dpi |
| JPEG | Qualité std | 300dpi | 300dpi |
| PDF | Qualité std | 300dpi | 300dpi |
| SVG | ✗ | ✓ | ✓ |
| CSV | ✗ | ✓ | ✗ |
| JSON | ✗ | ✓ | ✗ |
| HTML | ✗ | ✓ | ✗ |

#### Templates
| Template | Amateur | Producteur | Influenceur |
|----------|---------|-----------|------------|
| Compact | ✓ | ✓ | ✓ |
| Détaillé | ✓ | ✓ | ✓ |
| Complète | ✗ | ✓ | ✗ |
| Influenceur | ✗ | ✓ | ✓ |
| Personnalisé | ✗ | ✓ | ✓ |

#### Formats Canva
| Format | Amateur | Producteur | Influenceur |
|--------|---------|-----------|------------|
| 1:1 | ✓ | ✓ | ✓ |
| 16:9 | ✓ | ✓ | ✗ |
| 9:16 | ✓ | ✓ | ✓ |
| A4 | ✗ | ✓ | ✗ |

#### Personnalisation
| Personnalisation | Amateur | Producteur | Influenceur |
|-----------------|---------|-----------|------------|
| Thème clair/sombre | ✓ | ✓ | ✓ |
| Palette couleurs | ✓ | ✓ | ✓ |
| Polices web-safe | ✓ | ✓ | ✓ |
| Polices Google Fonts | ✓ | ✓ | ✓ |
| Filigrane | ✓ | ✓ | ✓ |
| Effets images | ✓ | ✓ | ✓ |
| Drag & drop layout | ✗ | ✓ | ✓ |

---

### PROFILS - Gestion Compte

| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Éditer profil | ✓ | ✓ | ✓ |
| Changer mot de passe | ✓ | ✓ | ✓ |
| Activer 2FA | ✓ | ✓ | ✓ |
| KYC optionnel | ✓ | ✓ | ✓ |
| Données entreprise | ✗ | ✓ | ✓ |
| Préférences avancées | ✓ | ✓ | ✓ |

---

### STATISTIQUES

| Stat | Amateur | Producteur | Influenceur |
|------|---------|-----------|------------|
| Reviews créées | ✓ | ✓ | ✓ |
| Exports réalisés | ✓ | ✓ | ✓ |
| Types produits | ✓ | ✓ | ✓ |
| Notes moyennes données | ✓ | ✓ | ✓ |
| Notes reçues | ✓ (publiques) | ✓ | ✓ |
| Engagements public | ✓ (publiques) | ✓ | ✓ |
| Stats cultures | ✗ | ✓ | ✗ |
| Rendements | ✗ | ✓ | ✗ |
| Stats détaillées | ✗ | ✓ | Limité |

---

### GALERIE PUBLIQUE

| Permission | Amateur | Producteur | Influenceur |
|-----------|---------|-----------|------------|
| Publier review | ✓ | ✓ | ✓ |
| Garder privée | ✓ | ✓ | ✓ |
| Voir galerie | ✓ | ✓ | ✓ |
| Liker reviews | ✓ | ✓ | ✓ |
| Commenter | ✓ | ✓ | ✓ |
| Partager | ✓ | ✓ | ✓ |

---

## 🔄 Logique d'Application

### Frontend Guards
```jsx
// Exemple de vérification de permission
const canEditPipeline = userTier === 'producteur';
const canExportSVG = ['producteur', 'influenceur'].includes(userTier);
```

### Backend Middleware
```javascript
// Middleware de vérification
function requireTier(tier) {
  return (req, res, next) => {
    if (req.user.tier !== tier) {
      return res.status(403).json({ error: 'Forbidden' });
    }
    next();
  };
}
```

### Routes API Protégées
- `POST /api/reviews` → Tous
- `POST /api/reviews/pipeline` → Producteur seulement
- `POST /api/export/svg` → Producteur/Influenceur
- `GET /api/cultivars` → Producteur seulement
- `GET /api/statistics/advanced` → Producteur seulement

---

## 📝 Notes Importantes

1. **Influenceur** = Focus sur rendu visuel/sharing, pas sur production technique
2. **Producteur** = Accès complet incluant données de production/rendement
3. **Amateur** = Utilisateur standard sans fonctionnalités avancées
4. Les données KYC restent optionnelles pour tous les niveaux
5. Toutes les reviews publiques sont commentables par tous (modération appliquée)

---

## 🔗 Références

- `server-new/middleware/auth.js` - Middleware authentification
- `server-new/middleware/permissions.js` - Middleware permissions
- `client/src/hooks/usePermissions.js` - Hook permissions
- `server-new/prisma/schema.prisma` - Modèle User
