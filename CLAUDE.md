...
- **Gap connu restant, non bloquant** : une review avec un pipeline très long (30+ étapes) dépasse toujours la hauteur d'une page non paginée — la fin du pipeline est coupée. La pagination (déjà livrée) est la réponse à ce cas ; `shouldAutoLockPagination`/le bandeau de suggestion (`TemplateSelector.jsx`, `PagedPreviewPane.jsx`) existent déjà mais n'ont pas été retestés spécifiquement sur ce scénario.
